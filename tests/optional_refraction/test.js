function configureTest(callback) {
    ...
    objects.push(new SceneObject(
        ...
        new FresnelPhongMaterial(Vec.of(0.827,0.412,0.424), 0.1, 0.2, 0, 100, 1.3)));
    objects.push(new SceneObject(
        new Sphere(Mat4.translation([-2, 1, -10]).times(Mat4.scale(2))),
        new FresnelPhongMaterial(Vec.of(0.255,0.506,0.498), 0.1, 0.2, 0, 100, 1.3)));
    objects.push(new SceneObject(
        new Sphere(Mat4.translation([5, 2, -12]).times(Mat4.scale(3))),
        new FresnelPhongMaterial(Vec.of(0.655,0.78,0.388), 0.1, 0.2, 0, 100, 1.3)));

    callback({
        renderer: new /*RandomMultisamplingRenderer*/IncrementalMultisamplingRenderer(new Scene(objects, lights, Vec.of(0.5,0.5,0.5)), camera, 16, 4),
        width: 600,
        height: 600
    });
}